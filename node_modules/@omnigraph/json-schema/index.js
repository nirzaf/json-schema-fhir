...
async function handleOperationResponseConfig(operationResponseConfig, { schemaHeaders, cwd, fetchFn, logger = new utils.DefaultLogger('handleOperationResponseConfig'), }) {
    if (operationResponseConfig.responseSchema) {
        ...
        return jsonMachete.AnySchema;
    }
}
async function getReferencedJSONSchemaFromOperations({ operations, cwd, schemaHeaders, ignoreErrorResponses, logger = new utils.DefaultLogger('getReferencedJSONSchemaFromOperations'), fetchFn, baseUrl, operationHeaders, queryParams, }) {
    ...
    for (const operationConfig of operations) {
        ...
        if ('responseByStatusCode' in operationConfig) {
            ...
            if (responseSchemas.length === 1) {
                ...
                rootTypeDefinition.properties[fieldName] = jsonMachete.AnySchema;
            }
            else {
        ...
        const interpolationKeys = stringInterpolation.getInterpolationKeys(...interpolationStrings);
...
async function getDereferencedJSONSchemaFromOperations({ operations, cwd = crossHelpers.process.cwd(), logger, fetchFn, schemaHeaders, ignoreErrorResponses, baseUrl, operationHeaders, queryParams, }) {
    ...
    const healedSchema = await jsonMachete.healJSONSchema(fullyDeferencedSchema, {
...
function linkResolver(linkObjArgs, actualResolver, root, args, context, info) {
    for (const argKey in linkObjArgs) {
        ...
        const actualValue = typeof argInterpolation === 'string'
            ? stringInterpolation.stringInterpolator.parse(argInterpolation, {
                root,
                args,
                context,
                info,
...
const responseMetadataType = new graphql.GraphQLObjectType({
    ...
    fields: {
        ...
        statusText: { type: graphql.GraphQLString },
    ...
    for (const operationConfig of operations) {
        ...
        if (isPubSubOperationConfig(operationConfig)) {
            ...
            field.subscribe = (root, args, context, info) => {
                ...
                let pubsubTopic = stringInterpolation.stringInterpolator.parse(operationConfig.pubsubTopic, interpolationData);
            ...
            if (crossHelpers.process.env.DEBUG === '1' || crossHelpers.process.env.DEBUG === 'fieldDetails') {
                field.description = `
            ...
            field.resolve = async (root, args, context, info) => {
                ...
                if ('binary' in operationConfig) {
                    ...
                    if (input != null) {
                        ...
                        if (contentType === null || contentType === void 0 ? void 0 : contentType.startsWith('application/x-www-form-urlencoded')) {
                            requestInit.body = qs.stringify(input, queryStringOptions);
                        }
                ...
                if (queryParams) {
                    for (const queryParamName in queryParams) {
                        ...
                        const interpolatedQueryParam = stringInterpolation.stringInterpolator.parse(queryParams[queryParamName].toString(), interpolationData);
                ...
                if (operationConfig.queryParamArgMap) {
                    for (const queryParamName in operationConfig.queryParamArgMap) {
                        ...
                        if (argValue != null) {
                            ...
                            const queryParamsString = qs.stringify(queryParamObj, opts);
                            fullPath += fullPath.includes('?') ? '&' : '?';
                            fullPath += queryParamsString;
                        }
                    }
                ...
                const fetch$1 = (context === null || context === void 0 ? void 0 : context.fetch) || globalFetch;
                if (!fetch$1) {
                ...
                const response = await fetch$1(fullPath, requestInit, context, {
                    ...info,
                    sourceName: name,
                });
                            ...
                            get() {
                                return new Proxy({}, {
                                    get(_, requestProp) {
                                        switch (requestProp) {
                                            case 'query':
                                                return qs.parse(fullPath.split('?')[1]);
                                            case 'path':
                                            ...
                                            case 'header':
                                                return utils.getHeadersObj(requestInit.headers);
                                            case 'body':
                            ...
                            get() {
                                return new Proxy({}, {
                                    get(_, responseProp) {
                                        switch (responseProp) {
                                            case 'header':
                                                return utils.getHeadersObj(response.headers);
                                            case 'body':
                                                return obj;
                                            case 'query':
                                                return qs.parse(fullPath.split('?')[1]);
...
function getValidTypeName({ schemaComposer, isInput, subSchema, }) {
    if (!subSchema.title) {
        throw new Error('Missing title for schema; ' + utils$1.inspect(subSchema));
    }
...
function getTypeResolverFromOutputTCs(ajv, outputTypeComposers, subSchemaAndTypeComposers, statusCodeOneOfIndexMap) {
    ...
    return function resolveType(data, context, info) {
        ...
        if (data.$response) {
            const error = new graphql.GraphQLError(`HTTP Error: ${data.$statusCode}`, undefined, undefined, undefined, undefined, undefined, {
                $url: data.$url,
                $method: data.$method,
        ...
        enter(subSchema, { path, visitedSubschemaResultMap }) {
            ...
            if (subSchema.const) {
                ...
                const typeComposer = schemaComposer.createEnumTC({
                    name: scalarTypeName,
                    values: {
                        [utils.sanitizeNameForGraphQL(subSchema.const)]: {
            ...
            if (subSchema.format) {
                switch (subSchema.format) {
                    case 'byte': {
                        const typeComposer = schemaComposer.getAnyTC(graphqlScalars.GraphQLByte);
                        return {
                            input: typeComposer,
                            output: typeComposer,
                            description: subSchema.description,
                    ...
                    case 'uuid': {
                        const typeComposer = schemaComposer.getAnyTC(graphqlScalars.GraphQLUUID);
                        return {
                            input: typeComposer,
                            output: typeComposer,
                            description: subSchema.description,
                    ...
                    case 'unix-time': {
                        const typeComposer = schemaComposer.createScalarTC(graphqlScalars.GraphQLTimestamp);
                        return {
                            input: typeComposer,
                            output: typeComposer,
                            description: subSchema.description,
                    ...
                    case 'int64': {
                        const typeComposer = schemaComposer.createScalarTC(graphqlScalars.GraphQLBigInt);
                        return {
                            input: typeComposer,
                            output: typeComposer,
                            description: subSchema.description,
                    ...
                    case 'int32': {
                        const typeComposer = schemaComposer.createScalarTC(graphql.GraphQLInt);
                        return {
                            input: typeComposer,
                            output: typeComposer,
                            description: subSchema.description,
                    ...
                    case 'decimal':
                    case 'float': {
                        const typeComposer = schemaComposer.createScalarTC(graphql.GraphQLFloat);
                        return {
                            input: typeComposer,
                            output: typeComposer,
                            description: subSchema.description,
            ...
            if (subSchema.minimum === 0) {
                const typeComposer = schemaComposer.getAnyTC(subSchema.type === 'integer' ? graphqlScalars.GraphQLNonNegativeInt : graphqlScalars.GraphQLNonNegativeFloat);
                return {
                    input: typeComposer,
                    output: typeComposer,
                    description: subSchema.description,
                ...
                const typeComposer = schemaComposer.getAnyTC(subSchema.type === 'integer' ? graphqlScalars.GraphQLPositiveInt : graphqlScalars.GraphQLPositiveFloat);
                return {
                    input: typeComposer,
                    output: typeComposer,
                    description: subSchema.description,
            ...
            if (subSchema.maximum === 0) {
                const typeComposer = schemaComposer.getAnyTC(subSchema.type === 'integer' ? graphqlScalars.GraphQLNonPositiveInt : graphqlScalars.GraphQLNonPositiveFloat);
                return {
                    input: typeComposer,
                    output: typeComposer,
                    description: subSchema.description,
                ...
                const typeComposer = schemaComposer.getAnyTC(subSchema.type === 'integer' ? graphqlScalars.GraphQLNegativeInt : graphqlScalars.GraphQLNegativeFloat);
                return {
                    input: typeComposer,
                    output: typeComposer,
                    description: subSchema.description,
            ...
            switch (subSchema.type) {
                ...
                case 'string': {
                    if (subSchema.minLength === 1 && subSchema.maxLength == null) {
                        const tc = schemaComposer.getAnyTC(graphqlScalars.GraphQLNonEmptyString);
                        return {
                            input: tc,
                            output: tc,
                            description: subSchema.description,
                    ...
                    const typeComposer = schemaComposer.getAnyTC(graphql.GraphQLString);
                    return {
                        input: typeComposer,
                        output: typeComposer,
                        description: subSchema.description,
        ...
        leave(subSchemaAndTypeComposers, { path }) {
            ...
            if (subSchemaAndTypeComposers.allOf) {
                ...
                for (const maybeTypeComposers of subSchemaAndTypeComposers.allOf) {
                    ...
                    if (inputTypeComposer instanceof graphqlCompose.ListComposer) {
                        isList = true;
                        inputTypeComposer = inputTypeComposer.ofType;
                    }
                    if (inputTypeComposer instanceof graphqlCompose.ScalarTypeComposer || inputTypeComposer instanceof graphqlCompose.EnumTypeComposer) {
                    ...
                    if (graphqlCompose.isSomeInputTypeComposer(outputTypeComposer)) {
                        ...
                        if (outputTypeComposer instanceof graphqlCompose.InterfaceTypeComposer) {
                ...
                if (ableToUseGraphQLInputObjectType) {
                    ...
                    subSchemaAndTypeComposers.input = schemaComposer.getAnyTC(graphqlScalars.GraphQLJSON);
                }
            }
            if (subSchemaAndTypeComposers.anyOf) {
                ...
                for (const typeComposers of subSchemaAndTypeComposers.anyOf) {
                    ...
                    if (inputTypeComposer instanceof graphqlCompose.ListComposer || outputTypeComposer instanceof graphqlCompose.ListComposer) {
                        isList = true;
                        inputTypeComposer = inputTypeComposer.ofType;
                        ...
                        for (const fieldName in inputTypeElemFieldMap) {
                            ...
                            if (inputFieldMap[fieldName]) {
                                inputFieldMap[fieldName] = {
                                    type: schemaComposer.getAnyTC(graphqlScalars.GraphQLJSON),
                                };
                            }
                            else {
                                const field = inputTypeElemFieldMap[fieldName];
                        ...
                        for (const fieldName in typeElemFieldMap) {
                            ...
                            fieldMap[fieldName] = {
                                ...
                                type: () => {
                                    ...
                                    const namedType = graphql.getNamedType(fieldType);
                                    if (existingField) {
                                        const existingFieldType = existingField.type();
                                        const existingNamedType = graphql.getNamedType(existingFieldType);
                                        if (existingNamedType.name !== namedType.name) {
                                            return schemaComposer.getAnyTC(graphqlScalars.GraphQLJSON);
                                        }
                                    }
            ...
            switch (subSchemaAndTypeComposers.type) {
                case 'object':
                    ...
                    if (Object.keys(fieldMap).length === 0) {
                        output = schemaComposer.getAnyTC(graphqlScalars.GraphQLJSON);
                    }
                    ...
                    if (Object.keys(inputFieldMap).length === 0) {
                        input = schemaComposer.getAnyTC(graphqlScalars.GraphQLJSON);
                    }
...
async function createBundle(name, { baseUrl, operations, schemaHeaders, operationHeaders, queryParams, cwd = process.cwd(), fetch: fetch$1 = fetch.fetch, logger = new utils.DefaultLogger(name), ignoreErrorResponses = false, }) {
