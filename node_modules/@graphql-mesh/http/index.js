...
function createMeshHTTPHandler({ baseDir, getBuiltMesh, logger, rawServeConfig = {}, playgroundTitle, }) {
    let readyFlag = false;
    ...
    const requestHandler = server.createServerAdapter({
        handleRequest: router.handle,
        baseObject: router,
    });
    router.all('/healthcheck', () => new fetch.Response(null, {
        status: readyFlag ? 204 : 503,
    }));
    router.all('/readiness', () => new fetch.Response(null, {
        status: readyFlag ? 204 : 503,
    }));
    ...
    if (staticFiles) {
        ...
        router.get('/', () => new fetch.Response(null, {
            status: 302,
            headers: {
    ...
    router.all(graphqlPath, ittyRouterExtras.withCookies, graphqlHandler(mesh$, playgroundTitle, playgroundEnabled, graphqlPath, corsConfig));
