import { process, path, fs } from '@graphql-mesh/cross-helpers';
...
function createMeshHTTPHandler({ baseDir, getBuiltMesh, logger, rawServeConfig = {}, playgroundTitle, }) {
    ...
    const { cors: corsConfig, staticFiles, playground: playgroundEnabled = process.env.NODE_ENV !== 'production', endpoint: graphqlPath = '/graphql',
    // TODO
    // trustProxy = 'loopback',
     } = rawServeConfig;
    const router = Router();
    ...
    router.all(graphqlPath, withCookies, graphqlHandler(mesh$, playgroundTitle, playgroundEnabled, graphqlPath, corsConfig));
